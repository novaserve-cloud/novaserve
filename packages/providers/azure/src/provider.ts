...
export class AzureProvider implements NovaProvider {
  ...
  async deploy(plan: DeploymentPlan): Promise<DeployResult> {
    ...
    for (const action of plan.actions) {
      ...
      deployedResources.push({
        ...
        id: `/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/nova-rg/providers/Microsoft.Web/sites/${action.resource.name}`,
  ...
  async destroy(resources: ResolvedResource[]): Promise<void> {
    // Implement destroy logic for Azure
  }
  ...
  async *getLogs(
    ...
    yield {
      ...
      level: "info",
      resource,
      message: "Azure Application Insights logs coming soon",
    };
  }
  ...
  async invoke(functionName: string, payload: unknown): Promise<InvokeResult> {
    return {
      statusCode: 200,
      body: { message: "Azure function invoked" },
      headers: {},
      durationMs: 0,
    };
  }
