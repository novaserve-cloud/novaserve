<p align="center">
  <img src="https://raw.githubusercontent.com/sazamansari/NovaServe-/main/docs/assets/logo.svg" height="80" alt="NovaServe Logo" />
</p>

# novaserve-provider-aws

> AWS Cloud deployment provider adapter for **NovaServe**.

Translates compiled Nova Intermediate Representation (Nova IR 1.0.0) graphs into AWS infrastructure resources using AWS SDK v3 (API Gateway v2, Lambda, S3, SQS, RDS PostgreSQL, EventBridge).

## Installation

```bash
npm install novaserve-provider-aws
# or
pnpm add novaserve-provider-aws
```

## Documentation & Repository

- **Main Repository**: [https://github.com/sazamansari/NovaServe-](https://github.com/sazamansari/NovaServe-)
- **Primary Package**: [https://www.npmjs.com/package/novaserve](https://www.npmjs.com/package/novaserve)

## License

Apache-2.0 © Md Shadab Azam Ansari & NovaServe Contributors
