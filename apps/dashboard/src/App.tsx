...
function JenkinsStyleLoginScreen({
  ...
  const handleSsoSignIn = (provider: string) => {
    ...
    setTimeout(() => {
      setSsoLoading(null);
      const mockJwt = `sso_header.${btoa(JSON.stringify({ sub: username, provider, role: 'administrator' }))}.sso_sig`;
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-gray-500 font-medium">
            Enterprise Cloud-Agnostic Serverless Development Platform
          </p>
        </div>
        ...
        {/* Error Alert Notice */}
        {errorMessage && (
          <div className="p-3.5 bg-red-50 border border-red-300 rounded-xl text-xs text-red-900 font-mono flex items-start gap-2.5 animate-in fade-in slide-in-from-top-2 duration-150 shadow-xs">
            <div className="text-red-600 font-bold shrink-0">⚠️</div>
            <div className="flex-1 font-medium">{errorMessage}</div>
            <button onClick={() => setErrorMessage(null)} className="text-red-500 hover:text-red-900 font-bold">✕</button>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-md p-6 sm:p-8 space-y-5">
          <div className="pb-3 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-gray-800">Production Workspace Authentication</h2>
              <p className="text-[11px] text-gray-500">Sign in with your verified enterprise credentials</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-semibold text-gray-700">Password / Workspace Secret Key</label>
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between text-xs text-gray-600">
              ...
              <span className="text-[11px] font-mono text-gray-400">JWT / HS256 Token</span>
            </div>

            <div className="space-y-2 pt-2">
              <button
                type="submit"
                disabled={isSubmitting || Boolean(ssoLoading)}
                className="btn-primary-yellow w-full justify-center py-2.5 text-xs shadow-xs font-semibold"
              >
                {isSubmitting ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin text-gray-800" />
                    <span>Verifying Credentials & JWT Token...</span>
                  </>
                ) : (
                  <>
          ...
          <div className="pt-2 border-t border-gray-100 text-center">
            <span className="text-[11px] text-gray-400 font-mono">Or authenticate via Single Sign-On (SSO)</span>
        ...
        <div className="text-center text-[11px] text-gray-400 font-mono space-y-1">
          <div className="flex items-center justify-center gap-1.5">
            ...
            <span>Protected by SOC2 Type II • Zero-Trust RBAC & Session Manager</span>
          </div>
          <div>NovaServe Engine v2.0.0 Production Cluster :4002</div>
        </div>
      </div>
    </div>
  );
